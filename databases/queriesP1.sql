...
INSERT INTO movies(title, year, director_id) VALUES('Amelie',2001,(SELECT director_id FROM directors WHERE first='Jean-Pierre' AND last ='Jeunet'));
...
SELECT country FROM directors JOIN movies ON directors.director_id = movies.director_id WHERE movies.title = 'Amelie';
...
SELECT movies.title, directors.first, directors.last FROM movies JOIN directors ON directors.director_id = movies.director_id ORDER BY directors.last ASC;